...
class NN_L2_setup():
    def __init__(self, Gempy_Inputs, PDE_outputs, Jacobian,Encoder_matrix, Decoder_matrix, bias, Mass_Matrix, Model="s", dtype=torch.float32, preferred_device: str = "auto" ):
        """
        Constructor Method for the NeuralNet class
        ...
        Args:
            ...
            Encoder_matrix (float) : Transformation Matrix to transform the data from lower dimension to orignial space. 
                                     It has shape (dr x dU).
        ...
        super(NN_L2_setup, self).__init__() # Initialialise the parent class
        ...
        if preferred_device == "mps":
            ...
            if torch.cuda.is_available():
                self.device = torch.device("cuda")
                print("Auto mode: Using CUDA device")
            elif torch.backends.mps.is_available() and torch.backends.mps.is_built():
                self.device = torch.device("mps")
                print("Auto mode: Using MPS device")
            else:
                self.device = torch.device("cpu")
                print("Auto mode: Using CPU device")

        else:
            raise ValueError(f"Unknown device option: {preferred_device}")
        ...
        self.ur = torch.einsum('ij,nj->ni', self.Encoder_matrix, self.PDE_outputs - self.bias)
        self.Dur = torch.einsum('ij,njk->nik', self.Encoder_matrix, self.Jacobian)
        ######################################################################################
        # Create the Dataset for Neural network
        ######################################################################################
        dataset = TensorDataset(self.Gempy_Inputs, self.ur, self.Dur, self.PDE_outputs, self.Jacobian)
    ...
    def train_network_M2(self, layer_sizes, num_epochs,  lr=0.002):
        ...
        for epoch in range(num_epochs):
            ...
            for inputs, ur, Dur, output_PDE , output_grad_PDE in self.train_loader:
                
                optimizer.zero_grad()
                outputs_NN = self.model(inputs)  # Forward pass
                #outputs = torch.matmul(outputs_NN, self.Decoder_matrix.T ) + self.bias
                #loss = custom_loss_without_grad(ur, outputs_NN, self.Mass_matrix) 
                loss = torch.mean(torch.sum((ur-outputs_NN)**2, dim=1))
        ...
        for inputs, ur, Dur, output_PDE , output_grad_PDE in self.test_loader:
    ...
    def train_network_Jacobian_full(self, layer_sizes, num_epochs, scale_factor=1, lr=0.002 ):
        ...
        for epoch in range(num_epochs):
            ...
            for inputs, ur, Dur, output_PDE , output_grad_PDE in self.train_loader:
                ...
                L_2_loss = torch.mean(torch.sum((ur-outputs_NN)**2, dim=1))
                Jacobian_NN = calculate_jacobian_full(inputs, outputs_NN)
                Frobenius_norm = torch.sum((Dur - Jacobian_NN)**2, dim=(1, 2))
                F_2_loss =  torch.mean(Frobenius_norm)
                loss = (L_2_loss + a *  F_2_loss)/2
                loss.backward()  # Backpropagation
                optimizer.step()  # Update weights
                epoch_train_loss += loss.item()
            ...
            # with torch.no_grad():
            for inputs, ur, Dur, output_PDE , output_grad_PDE in self.valid_loader:
                inputs.requires_grad_(True)
                outputs_NN = model(inputs)  # Forward pass
                L_2_loss = torch.mean(torch.sum((ur-outputs_NN)**2, dim=1))
                Jacobian_NN = calculate_jacobian_full(inputs, outputs_NN)
    ...
    def train_network_Jacobian_truncated(self, layer_sizes, num_epochs, scale_factor=1, ran_size=2, lr=0.002):   
        ######################################################################################
        # Find U and V by taking expextation of Jacobian
        ######################################################################################
        N, r, m = self.Dur.shape
        result_grad_sum_left = torch.zeros(r,r, dtype=self.dtype, device=self.device)  # Initialize a tensor to accumulate the sum
        result_grad_sum_right = torch.zeros(m,m, dtype=self.dtype, device=self.device)
        ...
        for epoch in range(num_epochs):
            ...
            for inputs, ur, Dur, output_PDE , output_grad_PDE in self.train_loader:
                ...
                for i in range(ran_iter):
                    ...
                    U_k_T_nabla_q = torch.matmul(U_k.T, Dur)
                    #U_k_T_nabla_q = torch.matmul(U_k.T, output_grad_PDE)
                    grad_U_k_phi_Output_NN = calculate_jacobian(U_k.T , inputs, outputs_NN)
                    ...
                    norm += torch.sum((U_k_T_nabla_q - grad_U_k_phi_Output_NN)**2, dim=(1, 2))
                matrix_norm_expectation = (norm * r )/ (k.shape[0] * ran_iter)
                L_2_loss = torch.mean(torch.sum((ur-outputs_NN)**2, dim=1))
                F_2_loss = torch.sum(matrix_norm_expectation)
                loss = (L_2_loss +  a * F_2_loss)/2
                loss.backward()  # Backpropagation
                optimizer.step()  # Update weights
                epoch_train_loss += loss.item()
